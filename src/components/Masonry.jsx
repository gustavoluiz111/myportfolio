...
const Masonry = ({
                ...
                items.map(
                    item =>
                        new Promise(resolve => {
                            const img = new Image();
                            img.onload = () => {
                                dims[item.id] = { width: img.naturalWidth, height: img.naturalHeight };
                                resolve();
                            };
                            img.onerror = () => {
                                dims[item.id] = { width: 1000, height: 1000 };
                                resolve();
                            };
                            img.src = item.img;
                        })
    ...
    const grid = useMemo(() => {
        if (!width || !imagesReady || Object.keys(dimensions).length === 0) return [];
